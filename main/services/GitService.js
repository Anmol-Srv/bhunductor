...
class GitService {
  ...
  registerHandlers(ipcMain) {
    ipcMain.handle(IPC_CHANNELS.GIT_GET_PROFILE, async () => {
      try {
        ...
        console.error('[GitService] Error getting profile:', error);
        return { success: false, error: error.message };
      }
    });

    ipcMain.handle(IPC_CHANNELS.GIT_GET_CONFIG, async (event, repoPath) => {
      try {
        ...
        console.error('[GitService] Error getting config:', error);
        return { success: false, error: error.message };
      }
    });

    ipcMain.handle(IPC_CHANNELS.GIT_GET_CHECKS, async (event, folderId, worktreeId) => {
      try {
    ...
    ipcMain.handle(IPC_CHANNELS.GIT_GET_LOG, async (event, repoPath, count = 50) => {
      try {
        if (!repoPath) return { success: false, error: 'No repo path provided' };
